...
{
  {Timer timer1("Start of program",  0.0,  0.0);
  int nx, ny, nz;
  int blockx, blocky;

  if      (argc==1) {nx=128;           ny=128;           nz=128;           blockx = 32;          blocky = 32;}
  ...
  {Timer timer2("Before cg_solver",  0.0, 0.0);
  try {
  cg_solver_block(&L, n, x, b, tol, maxIter, &resNorm, &numIter, blockx, blocky, 0);
